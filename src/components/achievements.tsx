...
export default function AchievementsPage() {
    ...
    const [language, setLanguageState] = useState<'en' | 'fr' | 'de' | 'it'>('en');
    ...
    const getUIText = (key: string) => {
        const texts = {
            title: { en: 'Achievements', fr: 'Succès', de: 'Erfolge', it: 'Obiettivi' },
            unlocked: { en: 'Unlocked', fr: 'Débloqué le', de: 'Freigeschaltet am', it: 'Sbloccato il' },
            back: { en: 'Back to Home', fr: "Retour à l'accueil", de: 'Zurück zur Startseite', it: 'Torna alla Home' },
            reset: { en: 'Reset Achievements', fr: 'Réinitialiser les succès', de: 'Erfolge zurücksetzen', it: 'Resetta Obiettivi' },
            alertTitle: { en: 'Are you sure?', fr: 'Êtes-vous sûr ?', de: 'Bist du sicher?', it: 'Sei sicuro?' },
            alertDesc: { en: 'This will permanently delete all your achievements and related progress data (including stats and errors) for the current language. This action cannot be undone.', fr: 'Cela supprimera définitivement tous vos succès et les données de progression associées (y compris les statistiques et les erreurs) pour la langue actuelle. Cette action ne peut pas être annulée.', de: 'Dadurch werden alle deine Erfolge und zugehörigen Fortschrittsdaten (einschließlich Statistiken und Fehler) für die aktuelle Sprache dauerhaft gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.', it: 'Questo eliminerà permanentemente tutti i tuoi obiettivi e i dati di progresso correlati (incluse statistiche ed errori) per la lingua corrente. Questa azione non può essere annullata.' },
            cancel: { en: 'Cancel', fr: 'Annuler', de: 'Abbrechen', it: 'Annulla' },
            confirmReset: { en: 'Reset', fr: 'Réinitialiser', de: 'Zurücksetzen', it: 'Resetta' },
        };
                     ...
                     <TooltipProvider>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {displayedAchievements.map((achievement: Achievement) => {
                                ...
                                const achievementName = (isFrench && achievement.name_fr) ? achievement.name_fr : (isGerman && achievement.name_de) ? achievement.name_de : (isItalian && achievement.name_it) ? achievement.name_it : achievement.name;
                                const achievementDescription = (isFrench && achievement.description_fr) ? achievement.description_fr : (isGerman && achievement.description_de) ? achievement.description_de : (isItalian && achievement.description_it) ? achievement.description_it : achievement.description;
