...
import { questions as initialQuestions, type Question } from "@/lib/questions-phrasal-verbs-fr";
  ...
  if (questions.length === 0 || currentQuestionIndex >= questions.length) {
    return (
        <Card className="w-full max-w-lg shadow-2xl">
            ...
            <CardContent>
                <p className="text-center text-xl">Votre score final est: {score} / {questions.length}</p>
            </CardContent>
            <CardFooter className="flex justify-center gap-4">
                <Button onClick={restartTest}>Rejouer</Button>
                 <Link href="/" passHref>
